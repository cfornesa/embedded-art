...
async function checkSlugAvailability(slug) {
  ...
  slugFeedback.innerHTML = '<span class="text-secondary">checking Checking availability...</span>';
  ...
  try {
    ...
    if (res.status === 404) {
      ...
      isSlugAvailable = true;
      slugFeedback.innerHTML = '<span class="text-success fw-semibold">OK Available</span>';
    } else if (res.ok) {
      // Slug is taken (piece exists)
      isSlugAvailable = false;
      slugFeedback.innerHTML = '<span class="text-danger fw-semibold">X Already taken - please choose a different slug</span>';
    } else {
      // Unknown error (connection issue, server error, etc.)
      slugFeedback.innerHTML = '<span class="text-warning">WARNING Could not verify availability</span>';
      isSlugAvailable = true; // Allow submission anyway
    }
  } catch (err) {
    slugFeedback.innerHTML = '<span class="text-warning">WARNING Network error - could not check availability</span>';
    isSlugAvailable = true; // Allow submission anyway
  }
}
...
if (!form) {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (slugNormalized && !isSlugAvailable) {
      setMsg("WARNING The slug you entered is already taken. Please choose a different slug or leave it blank to auto-generate.", "warning");
      if (slugEl) {
    ...
    if (!email || email.length === 0) {
      setMsg("WARNING Email address is required.", "warning");
      if (emailEl) {
    ...
    if (!emailRegex.test(email)) {
      setMsg("WARNING Please enter a valid email address.", "warning");
      if (emailEl) {
        emailEl.scrollIntoView({ behavior: 'smooth', block: 'center' });
        emailEl.focus();
      }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 409 && data.error && data.error.includes("Slug")) {
          setMsg(`WARNING ${data.error} Your form data has been preserved - just change the slug above and try again.`, "warning");
      ...
      let configSummary = "CONFIG CONFIGURATION BACKUP (copy for your records):\n\n";
