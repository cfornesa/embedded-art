...
echo function_exists('mail') ? "OK Available\n" : "X Not available\n";

// 2. Check if fsockopen exists (for SMTP)
echo "2. fsockopen() function: ";
echo function_exists('fsockopen') ? "OK Available\n" : "X Not available\n";
...
if ($hasSmtp) {
    ...
    if (strpos($smtpCfg['SMTP_HOST'], 'your-smtp-host') !== false) {
        echo "X Placeholder values detected (not configured)\n";
    } else {
        echo "OK Configured\n";
    }
} else {
...
try {
    ...
    if (empty($sendmailPath) || $sendmailPath === '/usr/sbin/sendmail -t -i') {
        echo "   WARNING Warning: Default sendmail path may not work on Replit\n";
    }
...
if (!$hasSmtp || strpos($smtpCfg['SMTP_HOST'] ?? '', 'your-smtp-host') !== false) {
    echo "WARNING SMTP is not properly configured.\n\n";
    echo "To enable email notifications, configure SMTP credentials:\n\n";
    echo "OPTION 1: Create aframe/app/lib/config.php with:\n";
    echo "<?php\n";
    echo "return [\n";
    ...
    echo "OK SMTP appears to be configured.\n";
